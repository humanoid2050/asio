...
{
public:
    static std::unique_ptr<asio_connection> make_connection(boost::asio::io_service & service, std::unique_ptr<deviceDescription> description, std::shared_ptr<connection_handler>handler = std::shared_ptr<connection_handler>())
    {
        switch (description->get_type())
        {
        case deviceDescription::deviceType::SERIAL:
            return std::unique_ptr<asio_connection> (new asio_serial(service, std::move(description)));
    ...
    static std::unique_ptr<asio_timer> make_timer(boost::asio::io_service & service, std::unique_ptr<deviceDescription> description)
    {
        switch (description->get_type())
        {
        case deviceDescription::deviceType::TIMER:
