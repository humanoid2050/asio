...
class stream_server : public asio_connection
{
public:
// inherited API
...
protected:    
    stream_server(boost::asio::io_service & io_service, std::unique_ptr<deviceDescription> description, std::size_t connection_count = 1)
        : asio_connection(io_service, std::move(description)), acceptor_(io_service), connection_count_(connection_count)
    {
    ...
    bool start_accept()
    {
        if (auto handler = handler_.lock())
        {
            ...
            auto conn = deviceFactory::make_connection(acceptor_->get_service(), std::move(client_desc), handler);
            
            acceptor_.async_accept(conn->get_socket(),
                boost::bind(stream_server<family_T>::handle_accept,this, std::move(conn), boost::asio::placeholders::error));
            return true;
        }
        return false;
    ...
    void handle_accept(std::shared_ptr<stream_connection<family_T>> conn, const boost::system::error_code& err)
    {
        if (!err)
        {
            //if no errors, kickstart the connection
            clients_.push_back(conn);
            conn->handle_connect(boost::asio::error::success);
            if (clients_.size() < connection_count_)
            {
    ...
    {
        clients_.remove_if([](std::unique_ptr<stream_connection<family_T>> cli){cli.is_disconnected()});
    }
    
    boost::asio::basic_socket_acceptor<family_T> acceptor_;
    std::size_t connection_count_;
    
    std::list<std::unique_ptr<stream_connection<family_T>>> clients_;
};


class tcp_server : public stream_server<boost::asio::ip::tcp>
{
public:
    tcp_server(boost::asio::io_service & io_service, std::unique_ptr<deviceDescription> description)
        : stream_server<boost::asio::ip::tcp>(io_service, std::move(description))
    {
...
protected:
    virtual bool connect()
    {
        ...
        boost::asio::ip::tcp::endpoint ep(tcp::v4(), std::static_pointer_cast<socketDescription>(description_)->get_port());
...
class local_stream_server : public stream_server<boost::asio::local::stream_protocol>
{
public:
    local_stream_server(boost::asio::io_service & io_service, std::unique_ptr<deviceDescription> description)
        : stream_server<boost::asio::local::stream_protocol>(io_service, std::move(description))
    {
...
protected:
    virtual bool connect()
    {
        ...
        boost::asio::local::stream_protocol::endpoint ep(std::static_pointer_cast<socketDescription>(description_)->get_local_addr());
        acceptor_.open();
        acceptor_.bind(ep);
        acceptor_.listen();
